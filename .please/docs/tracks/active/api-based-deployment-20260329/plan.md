...
- [ ] T-1: Add `@vercel/client` dependency (pinned version)
- [ ] T-2: Extend `ActionConfig` with new fields: `target`, `prebuilt`, `force`, `env`, `buildEnv`, `regions`, `archive`, `rootDirectory`, `autoAssignCustomDomains`, `customEnvironment`, `isPublic`, `withCache`
- [ ] T-3: Add new inputs to `action.yml` with descriptions and defaults
- [ ] T-4: Update `config.ts` — parse new inputs in `getActionConfig()`, add env/buildEnv multiline parser
- [ ] T-5: Unit tests for new config parsing (env multiline, boolean inputs, defaults)

### Phase 2: Core — Implement API deployment

- [ ] T-6: Implement `VercelApiClient.deploy()` using `@vercel/client` `createDeployment` — iterate async generator, log events, extract deployment URL from `ready`/`alias-assigned` event
- [ ] T-7: Build `gitMetadata` from GitHub Actions context (sha, ref, actor, commit message, remoteUrl)
- [ ] T-8: Map `ActionConfig` fields to `VercelClientOptions` + `DeploymentOptions`
- [ ] T-9: Unit tests for `VercelApiClient.deploy()` — mock `createDeployment` generator, test event handling, error cases, URL extraction

### Phase 3: Routing — Client selection logic

- [ ] T-10: Update `createVercelClient()` in `vercel.ts` — return `VercelApiClient` when `vercelArgs` is empty, `VercelCliClient` when non-empty
- [ ] T-11: Unit tests for client selection routing
- [ ] T-12: Update existing integration test — remove 'not implemented' assertion, add API deploy test

### Phase 4: Verification & Documentation

- [ ] T-13: Run full test suite, verify all existing tests pass
- [ ] T-14: Run `pnpm build` and verify `dist/` builds successfully
- [ ] T-15: Update README.md — document new inputs, deprecation notice for `vercel-args`
