...
export function getGithubCommentInput(): boolean | string {
...
export function addVercelMetadata(
  key: string,
  value: string | number,
  providedArgs: string[],
): string[] {
...
export function parseArgs(s: string): string[] {
  ...
  const matches = Array.from(s.matchAll(/'([^']*)'|"([^"]*)"|\S+/g))
...
export function joinDeploymentUrls(
...
export function buildCommentBody(
  ...
  const comment = githubComment !== undefined ? githubComment : getGithubCommentInput()
  const domains = aliasDomains !== undefined ? aliasDomains : (globalThis as any).aliasDomains || []
  ...
  return rawGithubComment
    ...
    .replace(
      /\{\{deploymentUrl\}\}/g,
      joinDeploymentUrls(deploymentUrl, domains),
    )
}
